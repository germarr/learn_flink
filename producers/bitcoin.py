...
def get_quote(coin:str='BTC/USD'):
    url = f"https://data.alpaca.markets/v1beta3/crypto/us/latest/bars?symbols={coin}"
    headers = {
        "accept": "application/json",
        "APCA-API-KEY-ID": alpaca_key,
        "APCA-API-SECRET-KEY": alpaca_secret
    }
    response = requests.get(url, headers=headers).json()['bars'][coin]
    ...
    export_data = {
        "name_coin": coin,
        "moment_of_day": dates_d['timestamp'],
        "open_market": response['o'],
        "high": response['h'],
        "low": response['l'],
        "close": response['c'],
        "volume": response['v'],
        "current_time": dates_d['current_time']
    }
    return export_data
...
def main(kafka_broker:str='kafka-broker:9092'):
    ...
    with app.get_producer() as producer:
        while True:
            quote = get_quote()
            print(f"Sending bitcoin data: {quote}")
            producer.produce(
                topic = "bitcoin_price_2",
                key="Bitcoin",
...
# "BTC/USD": {
#   "c": 105012.23,    // close price
#   "h": 105065.49,    // high price
#   "l": 105012.23,    // low price
#   "n": 1,            // number of trades
#   "o": 105020.995,   // open price
#   "t": "2025-01-26T15:54:00Z",  // bar timestamp
#   "v": 0.000559874,  // volume
#   "vw": 105065.49    // volume-weighted average price
# }